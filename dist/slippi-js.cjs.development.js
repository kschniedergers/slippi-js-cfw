...
function generateOverallStats({
  ...
  const inputsByPlayer = keyBy__default["default"](inputs, "playerIndex");
  ...
  const overall = settings.players.map(player => {
    ...
    const playerInputs = get__default["default"](inputsByPlayer, playerIndex) || {};
    const inputCounts = {
      buttons: get__default["default"](playerInputs, "buttonInputCount"),
      triggers: get__default["default"](playerInputs, "triggerInputCount"),
  ...
  _onCommand(data) {
    ...
    switch (command) {
      case exports.Command.GAME_START:
        ...
        forEach__default["default"](players, player => {
          if (player.type === 3) {
            return;
          }
